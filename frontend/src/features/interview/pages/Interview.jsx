...
// ✅ ADDED: Reusable error banner component
...
const Interview = () => {
    ...
    // ✅ ADDED: Pull `error` out of the hook alongside report/loading
    ...
    // ✅ ADDED: Error state — shown instead of infinite loading when fetch fails
    if (error) {
        ...
        const errorMessage = isQuotaError
            ? "⏳ AI quota exceeded for today. The free tier allows 20 requests per day. Please wait a few minutes or try again tomorrow."
            : isAuthError
                ? "🔒 Your session has expired. Please log in again."
                : isNotFound
                    ? "📭 This interview report was not found. It may have been deleted."
                    : `❌ Failed to load the interview report. ${error.message || 'Please try again.'}`;
    ...
    // ✅ ADDED: Separate guard for missing report after loading completes with no error
    if (!report) {
        return (
            <main className='loading-screen'>
                ...
                <button
                    className='button primary-button'
                    style={{ margin: '20px auto', display: 'block', maxWidth: '200px' }}
                    onClick={() => window.history.back()}
                >
                    Go Back
                </button>
                    ...
                    <div className='skill-gaps'>
                        ...
                        <div className='skill-gaps__list'>
                            {report.skillGaps?.map((gap, i) => (
                                <span key={i} className={`skill-tag skill-tag--${gap?.severity || 'mid'}`}>
                                    {gap?.skill}
                                </span>
                            ))}
                        </div>
